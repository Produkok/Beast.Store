...
if( $callbackSignature !== $signature ) {
    ...
    $merchantRef = mysqli_real_escape_string($conn,$data->merchant_ref);
    if($event == 'payment_status'){
        $payload = [
            'merchant_ref' => $merchantRef,
        ];
        $curlTripay = curl_init();
        curl_setopt_array($curlTripay, [
          CURLOPT_FRESH_CONNECT  => true,
          CURLOPT_URL            => $tripayRequest.'/merchant/transactions?'.http_build_query($payload),
        ...
        $responseTripay = curl_exec($curlTripay);
        $errorTripay = curl_error($curlTripay);
        //echo $responseTripay;
        curl_close($curlTripay);
        $hasilTripay = json_decode($responseTripay, true);
        if($hasilTripay['success'] == true){
            $statusBayar = $hasilTripay['data'][0]['status'];
            if($statusBayar == 'PAID'){
                $cektrx = mysqli_query($conn,"SELECT * FROM `tb_tripay` WHERE merchant_ref = '$merchantRef'") or die(mysqli_error());
                    ...
                    if($cs['status'] == 0){
                        ...
                        if($providerID == 4){
                            ...
                            if($hasil['result'] == 'true'){
                                ...
                                $insert_transaksi = mysqli_query($conn,"INSERT INTO `tb_transaksi` (`kd_transaksi`, `date`, `transaksi`, `total`, `saldo`, `note`, `providerID`, `jenis`, `metode`, `userID`, `status`) VALUES ('$merchantRef','$created_date','Pengembalian Dana','$amount',0,'Pengembalian Dana', '$providerID','3','999','$userID',2)") or die(mysqli_error());
...
WA/TELEGRAM : '.$su['no_hp'];
            ...
            else if($statusBayar == 'FAILED'){
                $update = mysqli_query($conn,"UPDATE `tb_order` SET status = 3 WHERE kd_transaksi = '$merchantRef'") or die(mysqli_error());
                $update_tripay = mysqli_query($conn,"UPDATE `tb_tripay` SET `status` = '$statusBayar' WHERE `merchant_ref` = '$merchantRef'") or die(mysqli_error());
            }
            else if($data->status == 'EXPIRED'){
                $update = mysqli_query($conn,"UPDATE `tb_order` SET status = 4 WHERE kd_transaksi = '$merchantRef'") or die(mysqli_error());
                $update_tripay = mysqli_query($conn,"UPDATE `tb_tripay` SET `status` = '$statusBayar' WHERE `merchant_ref` = '$merchantRef'") or die(mysqli_error());
            }
